...
function enhanceTextWithMask(text, mask, previousValue) {
  ...
  for (let maskPartIndex = 0; maskPartIndex < maskParts.length; maskPartIndex++) {
    let partString = [];
  ...
  if (finalString.length !== maskParts.length &&
      lastPart && lastMaskPart &&
...
function TextInputWithMask({ onChangeText, value, mask, ...rest }, ref) {
